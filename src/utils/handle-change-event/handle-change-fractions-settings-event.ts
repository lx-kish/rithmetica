 ...
 * @return {void}
 */
const handleChangeFractionsSettings = (index: number, previousStateSetting: IFractionsSetting[]) => (e: React.ChangeEvent<HTMLInputElement> | React.ChangeEvent<HTMLSelectElement>): void => {
  ...
  let fieldType = "";
  if (e.target.type === "radio" && e.target.name.indexOf("section") > -1) fieldType = "section";
  ...
  switch (fieldType) {
    ...
    case "section":
      currentTaskType = types.filter(type => type.section === e.target.value).find(type => type.name === previousStateSetting[index].name);

      store.dispatch(changeSetting({ index: index, name: "section", value: e.target.value }));
    ...
    case "problemType":
      currentTaskType = types.filter(type => {
        return type.operation === previousStateSetting[index].operation &&
