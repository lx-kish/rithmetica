...
const Problems: React.FC<IProps> = (props) => {
	 ...
	 */
	const rows = (arr: IFractionsProblem[][], cols: number) => [...Array(Math.ceil(arr.length / cols))];

	// chunk the problems into the array of rows
	const contentRows = (arr: IFractionsProblem[][], cols: number) => rows(arr, cols).map((row, idx: number) => arr.slice(idx * cols, idx * cols + cols));

	const getContent = contentRows(stateProblems, stateColumns).map((row, i) => (

		<div key={i} className="problem__row">
			{row.map((problem, j) => <Problem key={j} content={problem} stateProblemIndex={stateProblems.indexOf(problem)} />)}
		</div>
	));
