...
const operandsFactory = (name: string, operation: string): (operation: string, numberOfOperands: number) => number[] | undefined => {
  ...
  try {
    ...
    if (name === '1-digit same denominator simple fractions') return oneDigitSameDenominatorSimpleFractions;
    if (name === '1-digit same denominator mixed fractions') return oneDigitSameDenominatorMixedFractions;
    if (name === '1-digit same denominator mixed simplified fractions') return oneDigitSameDenominatorMixedSimplifiedFractions;
